...
namespace Infrastructure.Repository.MySQL {
    public class Repository : IRepository {
        ...
        public async Task<bool> isExist(string sql, IEnumerable<object> q) {
            try {
                var result = (List<Dictionary<string, object>>)(await selectFromQuery(sql, q));
        ...
        public async Task<QueryResult<T>> selectFromQuery<T>(string sql, IEnumerable<object> q) {
            ...
            var data = await this.selectFromQuery(sql, q);
            res.resultAsObject = _mapper.Map<List<T>>(data);
            return res;
        }

        private async Task<IEnumerable<Dictionary<string, object>>> selectFromQuery(string sql, IEnumerable<object> q) {
            IDBCommand.prepare(sql);
            if (q != null) { IDBCommand.bindValues((List<object>)q); }
